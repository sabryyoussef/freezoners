...
class Partner(models.Model):
    ...
    stage_id = fields.Many2one(
        ...
        index=True,
        tracking=True,
        default=lambda self: self.env['partner.stage'].search([], limit=1).id,
        copy=False
    )

    emirates_no_id = fields.Many2one('res.partner.document', string="Dummy Emirates ID")
    ...
    def prepare_shareholder(self):
        ...
        for rec in self:
            ...
            self.with_context(skip_custom_write=True).write({'shareholder_ids': lines})
