...
class Task(models.Model):
    ...
    task_document_ids = fields.One2many(
        "documents.document", "task_id", string="Task Documents"
    )
    document_count = fields.Integer(
        compute="_compute_document_count", string="Document Count", store=True
    )
    document_type_ids = fields.One2many(
        "task.document.lines", "task_id", string="Document Types"
    )
    document_required_type_ids = fields.One2many(
        "task.document.required.lines", "task_id", string="Required Documents"
    )
    ...
    document_required_type_processing_ids = fields.One2many(
        "task.document.required.lines", "task_id", string="Processing Documents"
    )
